...
export default function EcommerceLatestListings({ title, subheader, list, ...other }: Props) {
  return (
    <Card {...other}>
...
function ListingItem({ listing }: ListingItemProps) {
  const { name, coverUrl, price, priceSale } = listing;
  ...
  return (
    <Stack direction="row" spacing={2}>
      ...
      <ColorPreview limit={3} colors={listing.colors} />
    </Stack>
  );
}
